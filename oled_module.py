...
import time
import psutil
import modulo_logging as modulo_logging
from error_buffer import get_recent_errors, get_dropped_errors_count
    ...
    def _get_system_stats(self):
        ...
        try:
            ...
            try:
                ...
                if temps:
                    # Buscar temperatura del CPU en diferentes nombres posibles
                    if 'coretemp' in temps:  # Intel/AMD en Linux
                        temp_cpu = temps['coretemp'][0].current
                    elif 'cpu_thermal' in temps:  # Raspberry Pi
                        temp_cpu = temps['cpu_thermal'][0].current
                    elif 'k10temp' in temps:  # AMD Ryzen
                        temp_cpu = temps['k10temp'][0].current
                    elif 'zenpower' in temps:  # AMD Zen
                        temp_cpu = temps['zenpower'][0].current
                    else:
                        # Tomar la primera disponible
                        for name, entries in temps.items():
                            if entries:
                                temp_cpu = entries[0].current
                                break
                    
                    # Umbral de advertencia de temperatura
                    if temp_cpu and temp_cpu > 80:
                        temp_warning = True
    ...
    def display_main_screen(self):
        ...
        if stats:
            ...
            if stats['temperature'] is not None:
                temp_color = self.COLOR_WARNING if stats['temp_warning'] else self.COLOR_OK
                temp_text = f"Temp: {stats['temperature']:.1f}°C"
                if stats['temp_warning']:
                    temp_text += " ⚠"
                temp_bbox = draw.textbbox((0, 0), temp_text, font=self.font_small)
                temp_width = temp_bbox[2] - temp_bbox[0]
                draw.text(((self.width - temp_width) / 2, y_start + spacing), 
                    temp_text, font=self.font_small, fill=temp_color)
                y_offset = spacing * 2
            else:
                y_offset = spacing
            ...
            draw.text(((self.width - memory_width) / 2, y_start + y_offset), 
            ...
            draw.text(((self.width - disk_width) / 2, y_start + y_offset + spacing), 
                    disk_text, font=self.font_small, fill=disk_color)
            
            # Mostrar mensaje si hay alguna advertencia
            if stats['cpu_warning'] or stats['memory_warning'] or stats['disk_warning'] or stats['temp_warning']:
...
def run(stop_event):
    ...
    # No destruir ventanas aquí, se hace desde el sistema principal